...
///// </sumary>
"use strict";
app.controller("SituacaoeventoController", ['$scope', 'SituacaoeventoService', 'NotyService', function ($scope, SituacaoeventoService, notyService) {
		$scope.tblData = [];
        ...
        $scope.showModal = function (action) {
			...
			document.getElementById("modal-Situacaoevento").style.display = "block";
            $("#modal-Situacaoevento").removeClass().addClass("modal bounceInRight animated active");
            $(".header").removeClass("active");
            $(".modal").data("action", action);
        }

		$scope.exitModal = function () {
			$("#modal-Situacaoevento").removeClass().addClass("modal bounceOut animated");
		...
		$scope.Edit = function (row) {
			...
			$scope.$apply(function() {
			  $scope.Situacaoevento = row;
			});
            $("#name").val($scope.Situacaoevento.Name);
            if (!$scope.Situacaoevento.Active) {
        ...
        $scope.Save = function () {
		    ...
		    if (action === "new") {
		        SituacaoeventoService.Post($scope.Situacaoevento, function (response) {
		        ...
		        SituacaoeventoService.GetById($scope.Situacaoevento.Id, function (response) {
		            if (response.status === 200) {
		                if (!response.data) {
		                ...
		                var equal = angular.equals($scope.Situacaoevento, response.data[0]);
		                if (equal) {
		                ...
		                SituacaoeventoService.Put($scope.Situacaoevento, function (response) {
		                    if (response.status === 200 || response.status === 201) {
		...
		$scope.Delete = function (nRow) {
		    ...
		    notyService.Noty("Confirmacao", "Deseja excluir este registro: "+entity.Name+"? ", notyService.Type.Warning, true, function (confimation) {
		        if (confimation) {
		            SituacaoeventoService.Delete(entity, function (response) {
		                if (response.status === 200 || response.status === 201) {
