...
///// </sumary>
"use strict";
app.controller("TipoeventoController", ['$scope', 'TipoeventoService', 'NotyService', function ($scope, TipoeventoService, notyService) {
		$scope.tblData = [];
		$scope.Tipoevento = {};
        $scope.showModal = function (action) {
			...
			document.getElementById("modal-Tipoevento").style.display = "block";
            $("#modal-Tipoevento").removeClass().addClass("modal bounceInRight animated active");
            $(".header").removeClass("active");
            $(".modal").data("action", action);
        }

		$scope.exitModal = function () {
			$("#modal-Tipoevento").removeClass().addClass("modal bounceOut animated");
		...
		$scope.Edit = function (row) {
			...
			$scope.$apply(function() {
			  $scope.Tipoevento = row;
			});
            $("#name").val($scope.Tipoevento.Name);
            if (!$scope.Tipoevento.Active) {
        ...
        $scope.Save = function () {
		    ...
		    if (action === "new") {
		        TipoeventoService.Post($scope.Tipoevento, function (response) {
		        ...
		        TipoeventoService.GetById($scope.Tipoevento.Id, function (response) {
		            if (response.status === 200) {
		                if (!response.data) {
		                ...
		                var equal = angular.equals($scope.Tipoevento, response.data[0]);
		                if (equal) {
		                ...
		                TipoeventoService.Put($scope.Tipoevento, function (response) {
		                    if (response.status === 200 || response.status === 201) {
		...
		$scope.Delete = function (nRow) {
		    ...
		    notyService.Noty("Confirmacao", "Deseja excluir este registro: "+entity.Name+"? ", notyService.Type.Warning, true, function (confimation) {
		        if (confimation) {
		            TipoeventoService.Delete(entity, function (response) {
		                if (response.status === 200 || response.status === 201) {
