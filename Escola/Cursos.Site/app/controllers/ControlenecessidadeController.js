"use strict";
app.controller("ControlenecessidadeController", ['$scope', 'controlenecessidadeService', 'NotyService', function ($scope, controlenecessidadeService, notyService) {
    $scope.codigo = "Código";
    $scope.title = "Controle de Necessidade";
    $scope.tblData = [];
	$scope.controlenecessidade = {};
    $scope.showModal = function (action) {
		if(action === "new")
			$scope.controlenecessidade = {};

		document.getElementById("modal-controlenecessidade").style.display = "block";
        $("#modal-controlenecessidade").removeClass().addClass("modal bounceInRight animated active");
        $(".header").removeClass("active");
        $(".modal").data("action", action);
    }

	$scope.exitModal = function () {
		$("#modal-controlenecessidade").removeClass().addClass("modal bounceOut animated");
	...
	$scope.Edit = function (row) {
		...
		$scope.$apply(function() {
			$scope.controlenecessidade = row;
		});
        $("#name").val($scope.controlenecessidade.Name);
        if (!$scope.controlenecessidade.Active) {
    ...
    $scope.Save = function () {
		...
		if (action === "new") {
		    controlenecessidadeService.Post($scope.controlenecessidade, function (response) {
		    ...
		    controlenecessidadeService.GetById($scope.controlenecessidade.Id, function (response) {
		        if (response.status === 200) {
		            if (!response.data) {
		            ...
		            var equal = angular.equals($scope.controlenecessidade, response.data[0]);
		            if (equal) {
		            ...
		            controlenecessidadeService.Put($scope.controlenecessidade, function (response) {
		                if (response.status === 200 || response.status === 201) {
	...
	$scope.Delete = function (nRow) {
		...
		notyService.Noty("Confirmacao", "Deseja excluir este registro: "+entity.Name+"? ", notyService.Type.Warning, true, function (confimation) {
		    if (confimation) {
		        controlenecessidadeService.Delete(entity, function (response) {
		            if (response.status === 200 || response.status === 201) {
	...
	$scope.Columns = [
        			{ field: "Id", title: "Id", sortable: false, visible:false },
						{ field: "Incricaoid", title: "Incrição", sortable: false, visible:false },
						{ field: "Clienteid", title: "Cliente", sortable: false, visible:false },
						{ field: "Necessidadeid", title: "Necessidade", sortable: false, visible:false },
				];
