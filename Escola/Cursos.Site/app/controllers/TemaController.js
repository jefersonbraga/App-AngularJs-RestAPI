...
///// </sumary>
"use strict";
app.controller("temaController", ['$scope', 'temaService', 'NotyService','$route', function ($scope, temaService, notyService, $route) {
    $scope.codigo = "Código";
    $scope.title = "Temas";
    ...
    $scope.showModal = function (action) {
		...
		document.getElementById("modal-tema").style.display = "block";
        $("#modal-tema").removeClass().addClass("modal bounceInRight animated active");
        $(".header").removeClass("active");
        $(".modal").data("action", action);
    }

	$scope.exitModal = function () {
		$("#modal-tema").removeClass().addClass("modal bounceOut animated");
	...
	$scope.Edit = function (row) {
        ...
        $("#name").val($scope.Tema.Name);
        $scope.showModal("edit");
    }
		
	var getAll = function () {
        temaService.GetAll(function (response) {
    ...
    $scope.Save = function () {
		...
		if (action === "new") {
		    temaService.Post($scope.Tema, function (response) {
		    ...
		    temaService.GetById($scope.Tema.Id, function (response) {
		        if (response.status === 200) {
		            if (!response.data) {
		            ...
		            var equal = angular.equals($scope.Tema, response.data[0]);
		            if (equal) {
		            ...
		            temaService.Put($scope.Tema, function (response) {
		                if (response.status === 200 || response.status === 201) {
	...
	$scope.Delete = function (nRow) {
		...
		notyService.Noty("Confirmacao", "Deseja excluir este registro: "+entity.Name+"? ", notyService.Type.Warning, true, function (confimation) {
		    if (confimation) {
		        temaService.Delete(entity, function (response) {
		            if (response.status === 200 || response.status === 201) {
	...
	$scope.Columns = [
        			{ field: "Id", title: "Id", sortable: false, visible:false },
						{ field: "Tema", title: "Tema", sortable: true },
				];
